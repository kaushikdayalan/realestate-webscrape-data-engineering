...
| `real_estate/`       | A Python directory that contains code for your Dagster repository                 |
| `real_estate_tests/` | A Python directory that contains tests for `real_estate`                      |
...
pyenv virtualenv $PYTHON_VERSION real_estate
pyenv activate real_estate
```

**Conda**
```bash
export PYTHON_VERSION=X.Y.Z
conda create --name real_estate python=PYTHON_VERSION
...
Tests can be found in `real_estate_tests` and are run with the following command:

```bash
pytest real_estate_tests
```

As you create Dagster ops and graphs, add tests in `real_estate_tests/` to check that your
